...
export default function Base64Utility() {
    ...
    try {
      await new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
          try {
            if (typeof reader.result === 'string') {
              const decoded = decodeBase64(reader.result);
              const blob = new Blob([decoded], { type: 'application/octet-stream' });
              saveAs(blob, 'decoded_file');
              setError('');
              resolve(null);
            }
          } catch (err) {
  ...
  const handleGenerateHexDump = async () => {
    ...
    setOutput(`data:${mime};base64,${output}`);
    setError('');
  };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="card bg-base-100 shadow-xl">
        <div className="card-body p-6">
          <div className="tooltip tooltip-bottom w-fit" data-tip="Choose your conversion type">
            <div role="tablist" className="tabs tabs-lifted">
              <button
                role="tab"
                className={`tab tab-lg ${tab === 'text' ? 'tab-active' : ''}`}
                onClick={() => {
